    ...
    fun getFilePath(context: Context, fileUri: Uri): String? {
        ...
        if (Build.VERSION.SDK_INT >= 19 && DocumentsContract.isDocumentUri(context.applicationContext, uri)) {
            if (isExternalStorageDocument(uri)) {
                ...
                return context.getFilesDir().toString() + "/" + split[1]
        ...
        if ("content".equals(uri.scheme, ignoreCase = true)) {
            ...
            try {
                ...
                if (cursor != null) {
                    ...
                    val column_index2 = cursor.getColumnIndexOrThrow(MediaStore.Images.Media.DISPLAY_NAME)
                    if (cursor.moveToFirst()) {
                        path = cursor.getString(column_index)
                        name = cursor.getString(column_index2)
                        cursor.close()
                    }
                }
                PreferenceManager
